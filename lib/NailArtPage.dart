...
class _NailArtPageState extends State<NailArtPage> {
  ...
  Future<void> _fetchNailArts() async {
    setState(() {
      _loading = true;
      _error = null;
    });
    final box = await Hive.openBox('NailArts');
    ...
    try {
      final response = await http.get(Uri.parse('https://wishlist.lipslay.com/api/category?category=nail-art'));
