...
class GentsSalonPage extends StatefulWidget {
  const GentsSalonPage({super.key});

  @override
  State<GentsSalonPage> createState() => _GentsSalonPageState();
}

class _GentsSalonPageState extends State<GentsSalonPage> {
  String _searchText = '';
  ...
  Future<void> _fetchGentsSalons() async {
    setState(() {
      _loading = true;
      _error = null;
    });
    final box = await Hive.openBox('GentsSalons');
    final cacheKey = 'gents_salon_services';
    ...
    try {
      final response = await http.get(
        Uri.parse('$baseUrl/api/category?category=gents-salon'),
      );
